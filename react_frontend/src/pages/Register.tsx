...
import { apiRegister, apiProfile } from '@api/backend';
...
const Register: React.FC = () => {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiRegister(name.trim(), email.trim(), pwd);
      saveToken(res.token);
      // Optionally verify token by fetching profile (ignoring failures)
      try {
    ...
    <div className="container" style={{ maxWidth: 560 }}>
      <div className="card" style={{ padding: '1.5rem', marginTop: '1rem' }}>
        ...
        <form style={{ display: 'grid', gap: '.75rem', marginTop: '1rem' }} onSubmit={(e) => { e.preventDefault(); submit(); }}>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" style={{ padding: '.7rem .9rem', borderRadius: 10, border: '1px solid rgba(17,24,39,.12)' }} />
